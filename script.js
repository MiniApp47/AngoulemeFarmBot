document.addEventListener('DOMContentLoaded', function () {
    ...
    tg.ready();
    tg.expand();
    tg.setHeaderColor('#120f0d');
    tg.setBackgroundColor('#070707');
  // --- LIENS DE CONTACT ANGOULEMEFARM ---
  const contactLinks = [
    {
      name: 'TELEGRAM ✈️',
      url: 'https://t.me/angoulemefarmbydm2',
      id: 'telegram',
    ...
    {
      name: 'INSTAGRAM 📸',
      url: 'https://www.instagram.com/angoulemefarm?igsh=MTNlM3o5eDZvZnhpcw==',
      id: 'instagram',
      className: 'instagram',
      text: 'INSTAGRAM 📸',
      icon: '#icon-instagram'
 ...
 const appData = [
   ...
   function renderCategoryList() {
    ...
    if (filteredCategories.length === 0) {
        ...
        productListContainer.innerHTML = '<p class="no-results" style="grid-column: 1/-1; text-align: center; padding: 20px; color: #b0a68e;">Aucune catégorie ne correspond.</p>';
        return;
    }
 ...
 function populateFilters() {
    ...
    document.body.addEventListener('click', async function (e) {
        ...
        if (target.closest('#confirm-order-button')) {
            const telegramUrl = 'https://t.me/angoulemefarmbydm2';
            const message = formatOrderMessage().replace(/\*/g, '');

            try {
                await navigator.clipboard.writeText(message);
                showNotification('📋 Commande copiée ! Colle-la dans la conversation Telegram.');
            } catch (error) {
